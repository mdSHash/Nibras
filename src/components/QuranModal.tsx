...
export default function QuranModal({ reference, onClose }: QuranModalProps) {
  ...
  return (
    <AnimatePresence>
      {reference && data && (
        <div className="fixed inset-0 flex items-center justify-center p-4 sm:p-6 pointer-events-auto" style={{ zIndex: 9999 }} dir="rtl">
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div 
            ...
            className="relative w-full max-w-4xl max-h-[85vh] bg-parchment rounded-xl shadow-2xl overflow-hidden flex flex-col border border-accent/30"
          >
            {/* Decorative Header */}
          <div className="bg-[#f0ecd6] border-b border-accent/20 p-5 shrink-0 relative overflow-hidden">
            ...
            <button 
              onClick={onClose}
              className="absolute left-4 top-4 p-2 bg-ink/5 hover:bg-battle-red/10 hover:text-battle-red text-ink rounded-full transition-colors z-10"
              aria-label="إغلاق التفسير"
            >
              <X size={20} />
            </button>
            <div className="text-center">
              <h2 className="text-2xl font-bold text-accent mb-1">{reference}</h2>
          ...
          <div className="p-6 md:p-10 overflow-y-auto" style={{ backgroundImage: 'radial-gradient(circle at center, rgba(139, 107, 74, 0.03) 0%, transparent 100%)' }}>
            <p 
              className="text-[28px] md:text-[34px] leading-[2.6] text-center text-islamic-green" 
