...
const getEraTheme = (era?: string) => {
  ...
  if (era.includes('المكي') || era.includes('المدني') || era.includes('الوحي') || era.includes('البعثة') || era.includes('قبل البعثة')) return { color: '#10b981', title: 'عهد النبوة' }; // Emerald / Prophet ﷺ
  ...
  if (era.includes('الراشدة')) return { color: '#eab308', title: 'الخلافة الراشدة' }; // Yellow fallback
...
export default function Timeline({ events, selectedEvent, onSelectEvent, tourState }: TimelineProps) {
  ...
  const sortedEvents = [...events].sort((a, b) => a.date.gregorian - b.date.gregorian);
  ...
  const quickJumps = [
    { label: 'العهد النبوي', color: '#10b981', target: sortedEvents.find(e => e.era?.includes('الوحي') || e.era?.includes('المدني') || e.title.includes('نزول')) },
    { label: 'أبو بكر الصديق', color: '#fbbf24', target: sortedEvents.find(e => e.title.includes('تولي أبو بكر') || e.era?.includes('أبي بكر')) },
    { label: 'عمر بن الخطاب', color: '#ef4444', target: sortedEvents.find(e => e.title.includes('تولي عمر') || e.era?.includes('عمر')) },
    { label: 'عثمان بن عفان', color: '#06b6d4', target: sortedEvents.find(e => e.title.includes('تولي عثمان') || e.era?.includes('عثمان')) },
    { label: 'علي بن أبي طالب', color: '#818cf8', target: sortedEvents.find(e => e.title.includes('تولي علي') || e.era?.includes('علي')) },
  ];
  ...
  return (
    <>
        ...
        {quickJumps.map((jump, i) => (
          jump.target ? (
            <button
              ...
              style={{ borderBottomWidth: '2px', borderBottomColor: jump.color }}
            >
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: jump.color }}></div>
              {jump.label}
            </button>
          ) : null
        ))}
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[120px] bg-ink text-parchment z-[500] border-t-2 border-border-dark select-none overflow-hidden pointer-events-auto">
        ...
        <AnimatePresence>
          <motion.div 
            ...
            style={{ background: `radial-gradient(ellipse at bottom, ${eraTheme.color} 0%, transparent 60%)` }}
          />
        </AnimatePresence>
        ...
        >
          <div className="relative flex items-center h-full min-w-max px-10">
            ...
            <motion.div 
              ...
              animate={{ backgroundColor: eraTheme.color, boxShadow: `0 0 10px ${eraTheme.color}` }}
            ...
            <div className="flex items-center w-max gap-16 lg:gap-32 relative z-10 pt-6">
              {sortedEvents.map((evt, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute -top-[30px] text-[12px] ${isMajor ? 'opacity-100 font-bold text-accent' : 'opacity-70'} transition-opacity`}>
                    ...
                    <motion.div 
                      ...
                      animate={
                        isSelected && isTourMode
                          ? { 
                              scale: [isMajor ? 1.5 : 1, isMajor ? 2.3 : 1.8, isMajor ? 1.5 : 1], 
                              opacity: [1, 0.7, 1],
                              backgroundColor: [eventTheme.color, 'var(--color-parchment)', eventTheme.color]
                            }
                          : { 
                              scale: isSelected ? (isMajor ? 2 : 1.5) : (isMajor ? 1.4 : 1),
                              backgroundColor: isSelected ? 'var(--color-parchment)' : eventTheme.color,
                              boxShadow: isSelected ? `0 0 12px var(--color-parchment), 0 0 20px ${eventTheme.color}` : (isMajor ? `0 0 6px ${eventTheme.color}` : 'none')
                            }
                      }
                      transition={
                        isSelected && isTourMode 
                    ...
                    <div className={`mt-3 text-sm transition-all ${isSelected ? 'text-parchment font-bold scale-110 drop-shadow-md' : (isMajor ? 'text-parchment font-extrabold group-hover:scale-105' : 'text-parchment/80 group-hover:text-parchment')}`}>
                      {isMajor && <span className="mr-1" style={{ color: eventTheme.color }}>✦</span>}
                      {evt.title}
                    </div>
                  </div>
                );
